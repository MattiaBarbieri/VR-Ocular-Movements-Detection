...
class DataProcessing(object):
    ...
    def _read_data(self):
        # df = pd.read_excel('Data/RealterData1.xlsx')
        df = pd.read_excel(self.infile)

        self.time = df.loc[:, "time_unity"]

        self.eye_valid_L = df.loc[:, "eye_valid_L"]
        self.eye_valid_R = df.loc[:, "eye_valid_R"]

        # import GAZE PARAMETERS from LEFT EYE.
        self.gaze_origin_Lx = df.loc[:, "gaze_origin_L.x(mm)"]
        self.gaze_origin_Ly = df.loc[:, "gaze_origin_L.y(mm)"]
        self.gaze_origin_Lz = df.loc[:, "gaze_origin_L.z(mm)"]

        self.gaze_direct_Lx = df.loc[:, "gaze_direct_L.x"]
        self.gaze_direct_Ly = df.loc[:, "gaze_direct_L.y"]
        self.gaze_direct_Lz = df.loc[:, "gaze_direct_L.z"]

        self.pupil_position_Lx = df.loc[:, "pupil_position_L.x"]
        self.pupil_position_Ly = df.loc[:, "pupil_position_L.y"]

        self.gaze_contingency_Lx = df.loc[:, "gaze_contingency_L.x"]
        self.gaze_contingency_Ly = df.loc[:, "gaze_contingency_L.y"]


        # import GAZE PARAMETERS from RIGHT EYE.
        self.gaze_origin_Rx = df.loc[:, "gaze_origin_R.x(mm)"]
        self.gaze_origin_Ry = df.loc[:, "gaze_origin_R.y(mm)"]
        self.gaze_origin_Rz = df.loc[:, "gaze_origin_R.z(mm)"]

        self.gaze_direct_Rx = df.loc[:, "gaze_direct_R.x"]
        self.gaze_direct_Ry = df.loc[:, "gaze_direct_R.y"]
        self.gaze_direct_Rz = df.loc[:, "gaze_direct_R.z"]

        self.pupil_position_Rx = df.loc[:, "pupil_position_R.x"]
        self.pupil_position_Ry = df.loc[:, "pupil_position_R.y"]


        self.gaze_contingency_Rx = df.loc[:, "gaze_contingency_R.x"]
        self.gaze_contingency_Ry = df.loc[:, "gaze_contingency_R.y"]


        # import HEAD PARAMETERS.
        self.head_pos_x = df.loc[:, "head.position.x"]
        self.head_pos_y = df.loc[:, "head.position.y"]
        self.head_pos_z = df.loc[:, "head.position.z"]

        self.x = df.loc[:, "head.rotation.x"]
        self.y = df.loc[:, "head.rotation.y"]
        self.z = df.loc[:, "head.rotation.z"]
        self.w = df.loc[:, "head.rotation.w"]
    ...
    def plot_results(self):
        ...
        eye_L_x, eye_L_y, eye_R_x, eye_R_y, self.time = self.ImaokaConv()
